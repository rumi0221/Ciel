...
function showEvents(year, month, day) {
    ...
    eventDetails.style.transform = 'translateX(-50%) translateY(20px)';
    setTimeout(() => {
        eventDetails.style.transition = 'all 0.3s ease-out';
        eventDetails.style.opacity = '1';
        eventDetails.style.transform = 'translateX(-50%) translateY(0)';
    }, 10);
}
...
document.addEventListener('click', function(event) {
    ...
    // クリックされた要素がカレンダーテーブル内または詳細表示内でない場合
    if (!calendarTable.contains(event.target) && 
        !eventDetails.contains(event.target) && 
        !eventDetails.classList.contains('hidden')) {
